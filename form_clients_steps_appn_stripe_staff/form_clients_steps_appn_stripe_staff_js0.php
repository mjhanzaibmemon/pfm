...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_clients_steps_appn_stripe_staff']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['form_clients_steps_appn_stripe_staff']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="form_clients_steps_appn_stripe_staff"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
var Crtl_btn_snp = false;
function sc_btn_snp()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_snp(); }, 500);
      return;
    }
    if (Crtl_btn_snp) {return;}
    sc_btn_snp_ok();
}
function sc_btn_snp_cancel()
{
}
function sc_btn_snp_ok()
{
    Crtl_btn_snp = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?snp?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
var Crtl_btn_submit_app = false;
function sc_btn_submit_app()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_submit_app(); }, 500);
      return;
    }
    if (Crtl_btn_submit_app) {return;}
    scJs_confirm("<?php echo html_entity_decode('Have you completed the application,<br>including the required payment?', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_submit_app_ok, sc_btn_submit_app_cancel)
}
function sc_btn_submit_app_cancel()
{
}
function sc_btn_submit_app_ok()
{
    Crtl_btn_submit_app = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?submit_app?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
function sc_btn_requests()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_requests(); }, 500);
      return;
    }
    sc_btn_requests_ok();
}
function sc_btn_requests_cancel()
{
}
function sc_btn_requests_ok()
{
  nm_gp_submit('<?php echo $this->Ini->link_grid_vw_requests_cons; ?>', '<?php echo $this->nm_location; ?>', '<?php echo "link_remove_margin*scinno*scoutlink_remove_border*scinno*scoutlink_remove_background*scinno*scout"; ?>', 'inicio', '_self', '0', '0', 'grid_vw_requests');
}
function sc_btn_btn_back_reqs() 
{ 
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_btn_back_reqs(); }, 500);
      return;
    }
    window.location.href = '../grid_vw_requests/';
} 
function sc_btn_btn_back_reqs_cancel()
{
}
function sc_btn_btn_back_reqs_ok()
{
    window.location.href = '../grid_vw_requests/';
}
var Crtl_btn_approve_appn = false;
function sc_btn_approve_appn()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_approve_appn(); }, 500);
      return;
    }
    if (Crtl_btn_approve_appn) {return;}
    scJs_confirm("<?php echo html_entity_decode('Email notification? Status changes to Active; renewal date is updated.', ENT_COMPAT, $_SESSION['scriptcase']['charset']); ?>", sc_btn_approve_appn_ok, sc_btn_approve_appn_cancel)
}
function sc_btn_approve_appn_cancel()
{
}
function sc_btn_approve_appn_ok()
{
    Crtl_btn_approve_appn = true;
    document.F1.nmgp_parms.value = "nmgp_opcao?#?formphp?@?nm_call_php?#?approve_appn?@?";
    document.F1.action = "./";
    document.F1.target = "_self";
    document.F1.submit();
}
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_clients_steps_appn_stripe_staff']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_form_clients_steps_appn_stripe_staff_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinigual*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_form_clients_steps_appn_stripe_staff_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
function nm_mostra_doc(campo1, campo2, campo3)
{
    ...
    NovaJanela = window.open ("form_clients_steps_appn_stripe_staff_doc.php?script_case_init=<?php echo $this->form_encode_input($this->Ini->sc_page); ?>&nm_cod_doc=" + campo1 + "&nm_nome_doc=" + encodeURIComponent(campo2) + "&nm_cod_apl=" + campo3, "sc_name_download_iframe", "resizable, scrollbars");
}
...
{
    document.getElementById('form_clients_steps_appn_stripe_staff_form0').style.width='1px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form0').style.height='0px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form0').style.display='none';
    document.getElementById('form_clients_steps_appn_stripe_staff_form0').style.overflow='scroll';
    document.getElementById('form_clients_steps_appn_stripe_staff_form1').style.width='1px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form1').style.height='0px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form1').style.display='none';
    document.getElementById('form_clients_steps_appn_stripe_staff_form1').style.overflow='scroll';
    document.getElementById('form_clients_steps_appn_stripe_staff_form2').style.width='1px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form2').style.height='0px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form2').style.display='none';
    document.getElementById('form_clients_steps_appn_stripe_staff_form2').style.overflow='scroll';
    document.getElementById('form_clients_steps_appn_stripe_staff_form3').style.width='1px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form3').style.height='0px';
    document.getElementById('form_clients_steps_appn_stripe_staff_form3').style.display='none';
    document.getElementById('form_clients_steps_appn_stripe_staff_form3').style.overflow='scroll';
<?php
    if (!$_SESSION['sc_session'][$this->Ini->sc_page]['form_clients_steps_appn_stripe_staff']['pdf_view']) {
?>
...
?>
   ...
   if ("form_clients_steps_appn_stripe_staff_form2" == N_pagina) {
