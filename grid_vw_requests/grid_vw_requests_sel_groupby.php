<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_vw_requests']['glo_nm_path_imag_temp'] . "/sc_apl_default_PFM_Staff.txt"))
       {
?>
...
<?php
   ...
   $Sel_Groupby = new grid_vw_requests_sel_Groupby(); 
   $Sel_Groupby->Sel_Groupby_init();
   
class grid_vw_requests_sel_Groupby
{
function Sel_Groupby_init()
{
   ...
   if ($this->restore) {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['all']['SC_Ind_Groupby'];
   }
   else {
       $groupby_atual = $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Ind_Groupby'];
   }
...
function Sel_processa_out($sel_groupby)
{
   ...
   {
       if (isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['using_summary_cache'])) {
           unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['using_summary_cache']);
       }
       $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Ind_Groupby']     = $sel_groupby;
       $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['contr_array_resumo'] = "NAO";
       $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['contr_total_geral']  = "NAO";
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['ordem_quebra']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['ordem_select']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['tot_geral']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['Page_break_PDF']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['arr_total']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_group_by']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_x_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_y_axys']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_fill']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_order']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_order_col']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_order_level']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_order_sort']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_order_start']);
       unset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['pivot_tabular']);
   }
   if ($opc_ret == "resumo")
   {
       $summ_fields = explode("@?@", $this->summ_fields);
       if(isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'][ $sel_groupby ]))
       {
           foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'][ $sel_groupby ] as $i_sum => $d_sum)
           {
               $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'][ $sel_groupby ][$i_sum]['display'] = in_array($i_sum, $summ_fields);
           }
       }
       $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_order'][ $sel_groupby ] = $summ_fields;
       $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_drill_down'] = $this->drill_down;
   }
?>
    <script language="javascript"> 
<?php
...
function Sel_processa_form()
{
   ...
   $save_charset = $_SESSION['scriptcase']['charset'];
   if (!$embbed) {
       $_SESSION['scriptcase']['charset'] = "UTF-8";
   }
   ...
   $_SESSION['scriptcase']['charset'] = $save_charset;
...
<HEAD>
 <TITLE><?php echo $this->Nm_lang['lang_othr_grid_title'] ?> vw_client_appns</TITLE>
...
?>
 <script language="javascript" type="text/javascript">
  function scSubmitGroupBy(sPos) {
   $.ajax({
    type: "POST",
    url: "grid_vw_requests_sel_groupby.php",
    data: {
...
<?php
if ($this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
...
elseif (!$this->restore && $bSummaryPage && isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_control']))
{
    $aOptions = array();
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_control'] as $quebra_nome => $_arr_fields)
    {
        if(is_array($_arr_fields))
        {
            foreach ($_arr_fields as $_key_total => $d_field)
...
{
  $max_height = 1;
  if ($this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
  ...
  elseif (!$this->restore && isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display']))
  {
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'] as $_quebra_nome => $_i_sum)
    {
      if (count($_i_sum) > $max_height)
      {
          $max_height = count($_i_sum);
...
?>
   ...
   for (i = 0; i < fieldList.length; i++) {
    var fieldName;
    if ($("#" + fieldList[i]).find("select")[0]){
       fieldName = $("#" + fieldList[i]).find("select").val();
    } else {
       fieldName = $("#" + fieldList[i]).find("span").attr('data-sum');
    }
...
<?php
if ($bSummaryPage && $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Ind_Groupby'] == "sc_free_group_by") {
?>
   if (fieldList == "") {
...
<?php
    ...
    $iTabCount            = 0;
    foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_All_Groupby'] as $QB => $Tp)
    {
        if (!in_array($QB, $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Groupby_hide']))
        {
...
<?php
     if (!in_array("by_status", $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Groupby_hide']))
     {
         $this->static_gb_ok = "sc_id_sel_groupby_by_status";
        if ($groupby_atual == "by_status")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_by_status";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_by_status");display_btn_restore_gb();' name="sel_groupby" value="by_status" id="sc_id_sel_groupby_by_status"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_by_status">
        Membership Status
<?php
    $prep_labels = array();
    $prep_labels[] = "Membership Status";
?>
        <span>
...
?>
<?php
     if (!in_array("by_pmt_status", $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Groupby_hide']))
     {
        if ($groupby_atual == "by_pmt_status")
        {
            $check = " checked";
            $bStartFree = false;
            $this->static_gb_ok = "sc_id_sel_groupby_by_pmt_status";
        }
        else
        {
            $check = "";
...
?>
      <div>
      <input type="radio" class="scAppDivToolbarInput sc_ui_gby_selected" onclick='scShowTotalFields();change_actual_static("sc_id_sel_groupby_by_pmt_status");display_btn_restore_gb();' name="sel_groupby" value="by_pmt_status" id="sc_id_sel_groupby_by_pmt_status"<?php echo $check ?> style='display:' />
      <label for="sc_id_sel_groupby_by_pmt_status">
        Payment
<?php
    $prep_labels = array();
    $prep_labels[] = "Payment";
...
?>
<?php
     if (!in_array("_NM_SC_", $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Groupby_hide']))
     {
        if ($opc_ret != "resumo")
        {
...
<?php
    $prep_labels = array();
    $prep_labels[] = $this->Nm_lang['lang_groupby_none_selected'];
?>
        <span>
        <?php echo implode(', ', $prep_labels);?>
        </span>
...
<script>
    actual_static_gb = "<?php echo $this->static_gb_ok ?>";
</script>
<?php
   $iItemCount = 0;
   if ($bSummaryPage)
   {
       ...
       if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_control'])) {
           $arr_filds = array();
       }
       else {
           $arr_filds = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_control'] : $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_control'];
       }
    if (isset($arr_filds))
    {
?>
...
?>
  <td>
    <div class='scAppDivHeader scAppDivHeaderText SummaryBoxDiv  open'><?php echo $this->Nm_lang['lang_othr_totals']; ?></div>
...
<?php
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'])) {
              $arr_disp = array();
          }
          else {
                 $arr_disp = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_display'] : $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_display'];
          }
          if (!isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_order'])) {
              $arr_ord = array();
          }
          else {
              $arr_ord  = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['res']['summarizing_fields_order']   : $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['summarizing_fields_order'];
          }
...
<script type="text/javascript">
   ...
   if ($this->restore) {
       $oJson = new Services_JSON();
        ob_end_clean();
        if ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['all']['SC_Ind_Groupby'] == "sc_free_group_by") {
           $this->Arr_result['run_js'][] = "scShowFree()";
        } else {
           $this->Arr_result['set_var'][] = array('var' => 'actual_static_gb', 'value' => $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb']['all']['SC_Ind_Groupby']);
...
<?php
   $disp_rest = "none";
   foreach ($_SESSION['sc_session'][$sc_init]['grid_vw_requests']['dados_orig_gb'] as $tp => $fields) {
       foreach ($fields as $name => $data) {
           if (isset($_SESSION['sc_session'][$sc_init]['grid_vw_requests'][$name])) {
               if ($tp == "all" && ($name != "SC_Gb_Free_cmp" || $_SESSION['sc_session'][$sc_init]['grid_vw_requests']['SC_Ind_Groupby'] == "sc_free_group_by")) {
                   if ($data != $_SESSION['sc_session'][$sc_init]['grid_vw_requests'][$name]) {
                       $disp_rest = "";
                       break;
                   }
               }
               elseif ($tp == "res" && $bSummaryPage && $data != $_SESSION['sc_session'][$sc_init]['grid_vw_requests'][$name]) {
                   $disp_rest = "";
                   break;
               }
           }
...
function restore_gb() {
     ...
     .done(function(retcombos) {
        if (typeof deselectAllGrp == 'function') {deselectAllGrp()};
        ...
        $('.GroupByOptions input[checked]').click();
