...
class grid_vw_requests_total
{
   ...
   function __construct($sc_page)
   {
      ...
      if (isset($_SESSION['sc_session'][$this->sc_page]['grid_vw_requests']['campos_busca']) && !empty($_SESSION['sc_session'][$this->sc_page]['grid_vw_requests']['campos_busca']))
      { 
   ...
   //----- 
   function Calc_resumo_by_status($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang, $open_lft, $open, $paid;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['sql_tot_res']);
      ...
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('status' => "status");
      $cmps_quebra_atual = array("status");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb'];
      }
      $Save_Format = '';
      ...
      {
          if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_clear_total']) && $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_clear_total']) {
      ...
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_clear_total']);
      $ind_cmps  = 1;
      $ind_alias = "1";
      $cmp_dim   = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $this->Res_Totaliza_by_status($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          $Arr_tot_name = "array_total_" . $cmp_gb;
          $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['arr_total'][$cmp_gb] = $this->$Arr_tot_name;
      }
   }

   function Res_Totaliza_by_status($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      $cmp_sql_tp_num = array('client_id' => 'N','memb_status_id' => 'N','bus_cat_id' => 'N','bus_subcat_id' => 'N');
      ...
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['sql_tot_res']))
      {
         $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['sql_tot_res'] = str_replace("#@#cmps_quebras#@#", "", $comando);
      }
      $comando  = str_replace("#@#cmps_quebras#@#", "," . $cmps_quebras, $comando);
      $comando .= " group by " . $cmps_quebras . " order by " .  $cmps_quebras;
      if (!isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['Res_search_metric_use']) || empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['Res_search_metric_use']))
      {
          $comando = $cmd_simp;
          $cmps_S  = "";
      ...
      while (!$rt->EOF)
      {
          ...
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'by_status', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
          ...
          if ($Tem_estat_manual)
          {
              ...
              $vl_statistic = $this->Calc_statist_manual_by_status($where_ok);
   ...
   //----- 
   function Calc_resumo_by_pmt_status($destino_resumo, $res_export=false, $ind_compara="", $tipo="")
   {
      global $nm_lang, $open_lft, $open, $paid;
      $this->nm_data = new nm_data("en_us");
      unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['sql_tot_res']);
      ...
      $this->sc_where_atual  = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['where_pesq'];
      $ind_qb                = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_Ind_Groupby'];
      $cmp_sql_def   = array('payment_status' => "payment_status");
      $cmps_quebra_atual = array("payment_status");
      if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb']) && !empty($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb']))
      {
          $cmps_quebra_atual = $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['SC_force_fiels_gb'];
      }
      ...
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
          ...
          $arr_compara = "[" . $ind_compara . "]";
          $this->Res_Totaliza_by_pmt_status($ind_qb, $cmp_gb, $arr_tots . $arr_compara, $group, $join, $cmps_gb, $cmps_gb1, $cmps_gb2, $cmps_gbS, $cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format);
      }
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['arr_total'] = array();
      foreach ($cmps_quebra_atual as $cmp_gb)
      {
   ...
   function Res_Totaliza_by_pmt_status($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export, $Save_Format)
   {
      $sc_having = ((isset($parms_sub_sel['having']))) ? "  having " . $parms_sub_sel['having'] : "";
      $Tem_estat_manual = false;
      $where_ok = $this->sc_where_atual;
      ...
      $format_dimensions = array();
      $format_dimensions['payment_status']['reg'] = "S";
      $format_dimensions['payment_status']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              $prep_look  = $Cada_dim . "_SC_look";
              $$prep_look = $rt->fields[$Ind_sql];
              $SC_prep = $this->Ini->Get_format_dimension($Ind_sql, 'by_pmt_status', $Cada_dim, $rt, $format_dimensions[$Cada_dim]['reg'], $format_dimensions[$Cada_dim]['msk']);
              $SC_orig = $Cada_dim . "_orig";
              $SC_graf = "val_grafico_" . $Cada_dim;
              $$Cada_dim = $SC_prep['fmt'];
              $$SC_orig = $SC_prep['orig'];
              if ($Cada_dim == "payment_status") {
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
          ...
          {
              $where_ok = (empty($this->sc_where_atual)) ? " where " . $sql_where : $this->sc_where_atual . " and " . $sql_where;
              $vl_statistic = $this->Calc_statist_manual_by_pmt_status($where_ok);
              foreach ($vl_statistic as $ind => $val)
              {
                  $rt->fields[$ind] = $val;
              }
   ...
   //---- 
   function quebra_geral_by_status($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang , $client_id;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['contr_total_geral'] == "OK") 
      { 
          return; 
      } 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['tot_geral'] = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['tot_geral'][1] = $rt->fields[0] ; 
      $rt->Close(); 
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['contr_total_geral'] = "OK";
   } 

   //---- 
   function quebra_geral_by_pmt_status($res_limit=false, $res_export=false)
   {
      global $nada, $nm_lang , $client_id;
      if ($_SESSION['sc_session'][$this->Ini->sc_page]['grid_vw_requests']['contr_total_geral'] == "OK") 
      { 
   ...
   //-----  status
   function quebra_status_by_status($status, $arg_sum_status) 
   {
      global $tot_status, $client_id;
      $tot_status = array() ;  
      $tot_status[0] = $status ; 
   }
   //-----  payment_status
   function quebra_payment_status_by_pmt_status($payment_status, $arg_sum_payment_status) 
   {
      global $tot_payment_status, $client_id;
      $tot_payment_status = array() ;  
      $tot_payment_status[0] = $payment_status ; 
   }
