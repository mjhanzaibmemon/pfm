...
$old_error_handler = set_error_handler("control_steps_app_sent_mob_scriptcase_error_handler");

function control_steps_app_sent_mob_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['control_steps_app_sent_mob']['contr_erro']) && $_SESSION['scriptcase']['control_steps_app_sent_mob']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_control_steps_app_sent_mob->contr_control_steps_app_sent_mob->Apl_com_erro = true;
            if ($inicial_control_steps_app_sent_mob->contr_control_steps_app_sent_mob->NM_ajax_flag)
            {
                $inicial_control_steps_app_sent_mob->contr_control_steps_app_sent_mob->NM_ajax_info['errList']['geral_control_steps_app_sent_mob'][] = $msg;
                return;
            }
            control_steps_app_sent_mob_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function control_steps_app_sent_mob_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['control_steps_app_sent_mob'])  && '' != $_SESSION['scriptcase']['error_icon']['control_steps_app_sent_mob'])  ? $_SESSION['scriptcase']['error_icon']['control_steps_app_sent_mob']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['control_steps_app_sent_mob']) && '' != $_SESSION['scriptcase']['error_close']['control_steps_app_sent_mob']) ? $_SESSION['scriptcase']['error_close']['control_steps_app_sent_mob'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['control_steps_app_sent_mob']) && '' != $_SESSION['scriptcase']['error_span_title']['control_steps_app_sent_mob']) ? $_SESSION['scriptcase']['error_span_title']['control_steps_app_sent_mob'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['control_steps_app_sent_mob']) && '' != $_SESSION['scriptcase']['error_icon_title']['control_steps_app_sent_mob']) ? $_SESSION['scriptcase']['error_icon_title']['control_steps_app_sent_mob'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['control_steps_app_sent']['error_buffer'] .= ob_get_flush();
}
