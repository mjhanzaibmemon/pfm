...
class refin_search_modal
{
   function init_modal()
   {
       ...
       $label_cmp     = $_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['int_search_label'][$cmp_process];
       $lim_col       = 3;
       $qtd_col       = 0;
...
<?php
       $Cod_Btn_Apply = nmButtonOutput($this->arr_buttons, "bfilref_apply", "nm_proc_int_search_check()", "nm_proc_int_search_check()", "app_int_search_all", "", "", "", "absmiddle", "", "0px", $path_btn, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
       $bol_submit_onclick = false;
           $Cod_Btn_Cancel = nmButtonOutput($this->arr_buttons, "bfilref_limpar", "$('input[name=\'result[]\']').prop('checked', false); parent.$('input[name=\'int_search_{$cmp_process}[]\']').prop('checked', false);", "$('input[name=\'result[]\']').prop('checked', false); parent.$('input[name=\'int_search_{$cmp_process}[]\']').prop('checked', false);", "app_int_search_all_cancel", "", "", "", "absmiddle", "", "0px", $path_btn, "", "", "", "", "", "only_text", "text_right", "", "", "", "", "", "", "");
           ...
           $arr_field = $_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['int_search_dados'][$cmp_process]; 
                    foreach ($arr_field as $cada_opc => $dados_opc)
                    {
                        if(!isset($_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['interativ_search'][$cmp_process]['val_sel']) || !in_array($cada_opc, $_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['interativ_search'][$cmp_process]['val_sel']))
                        {
            ...
            <div class="scGridRefinedSearchMolduraResult scGridRefinedSearchCampo" style="overflow-y: scroll;">
                <div style="display: flex; flex-flow: row wrap;">
                    ...
                    foreach ($arr_field as $cada_opc => $dados_opc)
                    {
                        ...
                        $checked = (isset($_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['interativ_search'][$cmp_process]['val_sel']) && in_array($cada_opc, $_SESSION['sc_session'][$this->sc_init]['grid_vw_clients_main_member_inactive']['interativ_search'][$cmp_process]['val_sel'])) ? " checked" : "";
                        $evt  = "";
